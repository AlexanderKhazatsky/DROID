...
def convert_svo_to_mp4(filepath, recording_folderpath):
	...
	video_writer = cv2.VideoWriter(video_output_path,
		fourcc=video_codec, fps=15, frameSize=(width * 2, height))
	...
	for i in range(frame_count):
		...
		else: data_dict, timestamp = output
...
for folderpath in tqdm(all_folderpaths):
	...
	if not os.path.exists(mp4_folderpath): os.makedirs(mp4_folderpath)
	if not os.path.exists(svo_folderpath): os.makedirs(svo_folderpath)
