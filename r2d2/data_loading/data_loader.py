...
def create_dataloader(data_folderpaths, recording_prefix='MP4',
	...
	dataloader = DataLoader(shuffled_dataset,
		batch_size=batch_size, num_workers=num_workers, prefetch_factor=prefetch_factor)
	
	return dataloader

def create_train_test_data_loader(
		data_loader_kwargs={}, data_processing_kwargs={}, camera_kwargs={}
	):
	...
	train_dataloader = create_dataloader(train_folderpaths,
		**data_loader_kwargs, **data_processing_kwargs, camera_kwargs=camera_kwargs)
	test_dataloader = create_dataloader(test_folderpaths,
		**data_loader_kwargs, **data_processing_kwargs, camera_kwargs=camera_kwargs)
...
class TrainTestDataloader:
	...
	def __init__(self, data_loader_kwargs={}, data_processing_kwargs={}, camera_kwargs={}):
		...
		# Generate Train / Test Split #
		data_filtering_kwargs = data_loader_kwargs.pop('data_filtering_kwargs', {})
		self.train_folderpaths, self.test_folderpaths = \
			generate_train_test_split(**data_filtering_kwargs)

		# Create Dataloader Generator #
		self.generate_dataloader = lambda folderpaths: create_dataloader(folderpaths,
			**data_loader_kwargs, **data_processing_kwargs, camera_kwargs=camera_kwargs)
