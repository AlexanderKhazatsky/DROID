...
def collect_trajectory(env, controller=None, policy=None, horizon=None, save_filepath=None,
		metadata=None, wait_for_controller=False, obs_pointer=None, save_images=False,
	...
	num_steps = 0
	if reset_robot: env.reset(randomize=randomize_reset)
	...
	while True:
		...
		if save_filepath: traj_writer.write_timestep(timestep)
		...
		if end_traj:
			...
			if save_filepath: traj_writer.close(metadata=controller_info)
...
def replay_trajectory(env, filepath=None, assert_replayable_keys=['cartesian_position', 'gripper_position', 'joint_positions']):
	...
	for i in range(horizon):
		...
		# Follow Trajectory #
		if movement_enabled: env.step(action)
