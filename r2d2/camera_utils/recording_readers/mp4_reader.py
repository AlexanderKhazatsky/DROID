...
class MP4Reader:
	...
	def set_reading_parameters(self,
			image=True, concatenate_images=False,
		...
		if not os.path.isfile(timestamp_filepath): self._recording_timestamps = []
	...
	def _process_frame(self, frame):
		...
		if self.resolution == (0,0): return frame
	...
	def read_camera(self, ignore_data=False, correct_timestamp=None, return_timestamp=False):
		...
		try: received_time = self._recording_timestamps[self._index]
		except IndexError: received_time = None
